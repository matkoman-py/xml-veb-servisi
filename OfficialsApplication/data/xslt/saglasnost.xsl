<?xml version="1.0" encoding="UTF-8"?>
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                xmlns:sag="http://www.ftn.uns.ac.rs/Saglasnost"
                xmlns:fo="http://www.w3.org/1999/XSL/Format" version="2.0">

    <xsl:template match="/">
        <fo:root>
            <fo:layout-master-set>
                <fo:simple-page-master master-name="bookstore-page">
                    <fo:region-body margin="0.75in"/>
                </fo:simple-page-master>
            </fo:layout-master-set>

            <fo:page-sequence master-reference="bookstore-page">
                <fo:flow flow-name="xsl-region-body">
                    <fo:block font-family="sans-serif" font-size="1.4em" font-weight="bold">
                        SAGLASNOST ZA SPROVODJENJE
                    </fo:block>
                    <fo:block font-family="sans-serif" font-size="1.4em" font-weight="bold">
                        PREPORUCENE IMUNIZACIJE
                    </fo:block>
                    <fo:block font-family="sans-serif" font-size="1em">
                        (popunjava pacijent)
                    </fo:block>
                    <fo:block-container top="0in" position="absolute">
                        <fo:block text-align="right">
                            <fo:external-graphic
                                    src="url('data:image/jpeg;base64,')"/>
                        </fo:block>
                    </fo:block-container>
                    <fo:block font-family="sans-serif" font-size="0.7em" padding="5px" margin-top="0.5em">
                        <fo:inline font-weight="bold">
                            Drzavljanstvo
                        </fo:inline>
                        <fo:inline>
                            1) Republika Srbija | JMBG
                        </fo:inline>
                        <fo:inline>
                            <xsl:value-of select="concat(' ',sag:Saglasnost/sag:Drzavljanstvo/sag:JMBG)"/>
                        </fo:inline>
                    </fo:block>
                    <fo:block font-family="sans-serif" font-size="0.7em" padding="5px" margin-top="0.5em">
                        <fo:inline font-weight="bold">Prezime</fo:inline>
                        <fo:inline>
                            <xsl:value-of select="concat(' ',sag:Saglasnost/sag:Podaci_o_pacijentu/sag:Prezime)"/>
                        </fo:inline>
                        |
                        <fo:inline font-weight="bold">Ime</fo:inline>
                        <fo:inline>
                            <xsl:value-of select="concat(' ',sag:Saglasnost/sag:Podaci_o_pacijentu/sag:Ime)"/>
                        </fo:inline>
                        |
                        <fo:inline font-weight="bold">Ime roditelja</fo:inline>
                        <fo:inline>
                            <xsl:value-of select="concat(' ',sag:Saglasnost/sag:Podaci_o_pacijentu/sag:Ime_roditelja)"/>
                        </fo:inline>
                    </fo:block>
                    <fo:block font-family="sans-serif" font-size="0.7em" padding="5px" margin-top="0.5em">
                        <fo:inline font-weight="bold">Pol:</fo:inline>
                        <fo:inline>
                            <xsl:value-of select="concat(' ',sag:Saglasnost/sag:Podaci_o_pacijentu/sag:Pol)"/>
                        </fo:inline>
                        |
                        <fo:inline font-weight="bold">Datum rodjenja</fo:inline>
                        <fo:inline>
                            <xsl:value-of select="concat(' ',sag:Saglasnost/sag:Podaci_o_pacijentu/sag:Datum_rodjenja)"/>
                        </fo:inline>
                        |
                        <fo:inline font-weight="bold">Mesto rodjenja</fo:inline>
                        <fo:inline>
                            <xsl:value-of select="concat(' ',sag:Saglasnost/sag:Podaci_o_pacijentu/sag:Mesto_rodjenja)"/>
                        </fo:inline>
                    </fo:block>
                    <fo:block font-family="sans-serif" font-size="0.7em" padding="5px" margin-top="0.5em">
                        <fo:inline font-weight="bold">Adresa (ulica i broj)</fo:inline>
                        <fo:inline>
                            <xsl:value-of select="concat(' ',sag:Saglasnost/sag:Podaci_o_pacijentu/sag:Adresa/sag:Ulica_i_broj)"/>
                        </fo:inline>
                        |
                        <fo:inline font-weight="bold">Mesto/Naselje</fo:inline>
                        <fo:inline>
                            <xsl:value-of select="concat(' ',sag:Saglasnost/sag:Podaci_o_pacijentu/sag:Adresa/sag:Mesto)"/>
                        </fo:inline>
                    </fo:block>
                    <fo:block font-family="sans-serif" font-size="0.7em" padding="5px" margin-top="0.5em">
                        <fo:inline font-weight="bold">Opstina/Grad</fo:inline>
                        <fo:inline>
                            <xsl:value-of select="concat(' ',sag:Saglasnost/sag:Podaci_o_pacijentu/sag:Adresa/sag:Opstina)"/>
                        </fo:inline>
                        |
                        <fo:inline font-weight="bold">Tel. fiksni</fo:inline>
                        <fo:inline>
                            <xsl:value-of select="concat(' ',sag:Saglasnost/sag:Podaci_o_pacijentu/sag:Kontakt/sag:Fiksni)"/>
                        </fo:inline>
                    </fo:block>
                    <fo:block font-family="sans-serif" font-size="0.7em" padding="5px" margin-top="0.5em">
                        <fo:inline font-weight="bold">Tel. mobilni</fo:inline>
                        <fo:inline>
                            <xsl:value-of select="concat(' ',sag:Saglasnost/sag:Podaci_o_pacijentu/sag:Kontakt/sag:Mobilni)"/>
                        </fo:inline>
                        |
                        <fo:inline font-weight="bold">email</fo:inline>
                        <fo:inline>
                            <xsl:value-of select="concat(' ',sag:Saglasnost/sag:Podaci_o_pacijentu/sag:Kontakt/sag:Email)"/>
                        </fo:inline>
                    </fo:block>
                    <fo:block font-family="sans-serif" font-size="0.7em" padding="5px" margin-top="0.5em">
                        <fo:inline font-weight="bold">Radni status:</fo:inline>
                        <fo:inline>
                            <xsl:value-of select="concat(' ',sag:Saglasnost/sag:Podaci_o_pacijentu/sag:Radni_status)"/>
                        </fo:inline>
                    </fo:block>
                    <fo:block font-family="sans-serif" font-size="0.7em" padding="5px" margin-top="0.5em">
                        <fo:inline font-weight="bold">Zanimanje zaposlenog:</fo:inline>
                        <fo:inline>
                            <xsl:value-of select="concat(' ',sag:Saglasnost/sag:Podaci_o_pacijentu/sag:Zanimanje)"/>
                        </fo:inline>
                    </fo:block>
                    <fo:block font-family="sans-serif" font-size="0.7em" padding="5px" margin-top="0.5em">
                        <fo:inline font-weight="bold">Korisnik ustanove soc. zast.</fo:inline>
                        <xsl:value-of select="concat(' ',sag:Saglasnost/sag:Podaci_o_pacijentu/sag:Socijalna_zastita_info/sag:Koristi_zastitu)"/>
                        |Naziv_opstina_sedista
                        <fo:inline font-weight="bold">
                        </fo:inline>
                        <xsl:value-of select="concat(' ',sag:Saglasnost/sag:Podaci_o_pacijentu/sag:Socijalna_zastita_info/sag:Naziv_i_opstina_sedista)"/>
                    </fo:block>
                    <fo:block font-family="sans-serif" font-size="0.7em" padding="5px" margin-top="0.5em">
                        <fo:inline font-weight="bold">Izjavljujem da: </fo:inline>
                        <fo:inline>
                            <xsl:value-of select="concat(' ',sag:Saglasnost/sag:Podaci_o_pacijentu/sag:Izjava_saglasnosti/sag:Saglasan)"/>
                            sa sprovodjenjem aktivne/pasivne imunizacije (naziv imunoloskog leka):
                            <xsl:value-of select="concat(' ',sag:Saglasnost/sag:Podaci_o_pacijentu/sag:Izjava_saglasnosti/sag:Naziv_leka)"/>
                        </fo:inline>
                    </fo:block>
                    <fo:block font-weight="bold" font-family="sans-serif" font-size="0.9em" padding="5px" margin-top="0.5em">
                        Lekar mi je objasnio prednosti i rizike od sprovodjenja aktivne/pasivne imunizacije navedenim
                        imunoloskim lekom.
                    </fo:block>
                    <fo:block font-weight="bold" font-family="sans-serif" font-size="0.9em" padding="5px" margin-top="0.5em">
                        <fo:inline>Potpis pacijenta ili zakonskog zastupnika pacijenta</fo:inline>
                    </fo:block>
                    <fo:block font-family="sans-serif" font-size="0.9em" padding="5px" margin-top="-2em" margin-left="33em">
                        <fo:inline>Datum</fo:inline>
                    </fo:block>
                    <fo:block font-family="sans-serif" font-size="0.9em" padding="5px" margin-top="1em">
                        <fo:inline>__________________________________________________</fo:inline>
                    </fo:block>
                    <fo:block font-family="sans-serif" font-size="0.9em" padding="5px" margin-top="-2em" margin-left="33em">
                        <fo:inline>__.__._____.</fo:inline>
                    </fo:block>
                    <fo:block font-family="sans-serif" font-size="0.4em" padding="5px" margin-top="1em" >
                        |||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
                    </fo:block>
                    <fo:block margin-left="2em" font-family="sans-serif" font-size="1.5em" padding="5px" margin-top="0.5em">
                        EVIDENCIJA O VAKCINACIJI PROTIV COVID-19
                    </fo:block>
                    <fo:block margin-left="15em" font-family="sans-serif" font-size="0.9em" padding="5px" margin-top="-0.5em">
                        (popunjava zdravstveni radnik)
                    </fo:block>
                    <fo:block font-family="sans-serif" font-size="0.9em" padding="5px" margin-top="0.5em">
                        <fo:inline>Zdravstvena ustanova</fo:inline>
                        <fo:inline>
                            <xsl:value-of select="concat(concat(' ',sag:Saglasnost/sag:Evidencija_o_vakcinaciji/sag:Naziv_ustanove), ' ')"/>
                        </fo:inline>
                        <fo:inline>Vakcinacijski punkt</fo:inline>
                        <fo:inline>
                            <xsl:value-of select="concat(concat(' ',sag:Saglasnost/sag:Evidencija_o_vakcinaciji/sag:Vakcinacijski_punkt), ' ')"/>
                        </fo:inline>
                    </fo:block>
                    <fo:block font-family="sans-serif" font-size="0.9em" padding="5px" margin-top="-0.5em">
                        <fo:inline>Ime, prezime, faksimil i br. telefona lekara:</fo:inline>
                        <fo:inline>
                            <xsl:value-of select="concat(concat(' ',sag:Saglasnost/sag:Evidencija_o_vakcinaciji/sag:Podaci_lekar), ' ')"/>
                        </fo:inline>
                    </fo:block>
                    <fo:block font-family="sans-serif" font-size="0.8em">Pre davanja vakcine pregledati osobu i upoznati je sa koristima i o mogucim nezeljenim reakcijama posle
                        vakcinacije. Obavezno upisati datu vakcinu i sve trazene podatke u ovaj obrazac i podatke uneti u licni karton o izvrsenim imunizacijama i zdravstveni karton</fo:block>
                    <fo:block font-size="10px">
                        <fo:table font-family="serif" border="1px" margin-top="1em">
                            <fo:table-column column-width="7em" border="1px solid black"/>
                            <fo:table-column column-width="5em" border="1px solid black"/>
                            <fo:table-column column-width="6em" border="1px solid black"/>
                            <fo:table-column column-width="6em" border="1px solid black"/>
                            <fo:table-column column-width="6em" border="1px solid black"/>
                            <fo:table-column column-width="6em" border="1px solid black"/>
                            <fo:table-column column-width="6em" border="1px solid black"/>
                            <fo:table-column column-width="6.7em" border="1px solid black"/>
                            <fo:table-body>
                                <fo:table-row border="1px solid black">
                                    <fo:table-cell font-family="sans-serif" padding="2px">
                                        <fo:block>Naziv vakcine</fo:block>
                                    </fo:table-cell>
                                    <fo:table-cell font-family="sans-serif" padding="2px">
                                        <fo:block>Datum davanja vakcine</fo:block>
                                    </fo:table-cell>
                                    <fo:table-cell font-family="sans-serif" padding="2px">
                                        <fo:block>Nacin davanja vakcine</fo:block>
                                    </fo:table-cell>
                                    <fo:table-cell font-family="sans-serif" padding="2px">
                                        <fo:block>Ekstremitet</fo:block>
                                    </fo:table-cell>
                                    <fo:table-cell font-family="sans-serif" padding="2px">
                                        <fo:block>Serija vakcine (lot)</fo:block>
                                    </fo:table-cell>
                                    <fo:table-cell font-family="sans-serif" padding="2px">
                                        <fo:block>Proizvodjac</fo:block>
                                    </fo:table-cell>
                                    <fo:table-cell font-family="sans-serif" padding="2px">
                                        <fo:block>Nezeljena reakcija</fo:block>
                                    </fo:table-cell>
                                    <fo:table-cell font-family="sans-serif" padding="2px">
                                        <fo:block>Potpis lekara</fo:block>
                                    </fo:table-cell>
                                </fo:table-row>
                                <fo:table-row border="1px solid black">
                                    <fo:table-cell padding="2px">
                                        <fo:block>
                                            <xsl:value-of select="sag:Saglasnost/sag:Evidencija_o_vakcinaciji/sag:Vakcinacija/sag:Naziv_vakcine"/>
                                        </fo:block>
                                    </fo:table-cell>
                                    <fo:table-cell padding="2px">
                                        <fo:block>
                                            <xsl:value-of select="sag:Saglasnost/sag:Evidencija_o_vakcinaciji/sag:Vakcinacija/sag:Datum_vakcinacije"/>
                                        </fo:block>
                                    </fo:table-cell>
                                    <fo:table-cell padding="2px">
                                        <fo:block>
                                            <xsl:value-of select="sag:Saglasnost/sag:Evidencija_o_vakcinaciji/sag:Vakcinacija/sag:Naziv_vakcine"/>
                                        </fo:block>
                                    </fo:table-cell>
                                    <fo:table-cell padding="2px">
                                        <fo:block>
                                            <xsl:value-of select="sag:Saglasnost/sag:Evidencija_o_vakcinaciji/sag:Vakcinacija/sag:Ekstremitet"/>
                                        </fo:block>
                                    </fo:table-cell>
                                    <fo:table-cell padding="2px">
                                        <fo:block>
                                            <xsl:value-of select="sag:Saglasnost/sag:Evidencija_o_vakcinaciji/sag:Vakcinacija/sag:Lot"/>
                                        </fo:block>
                                    </fo:table-cell>
                                    <fo:table-cell padding="2px">
                                        <fo:block>
                                            <xsl:value-of select="sag:Saglasnost/sag:Evidencija_o_vakcinaciji/sag:Vakcinacija/sag:Proizvodjac"/>
                                        </fo:block>
                                    </fo:table-cell>
                                    <fo:table-cell padding="2px">
                                        <fo:block>
                                            <xsl:value-of select="sag:Saglasnost/sag:Evidencija_o_vakcinaciji/sag:Vakcinacija/sag:Nezeljena_reakcija"/>
                                        </fo:block>
                                    </fo:table-cell>
                                </fo:table-row>
                                <fo:table-row border="1px solid black">
                                    <fo:table-cell padding="2px">
                                        <fo:block>
                                            <xsl:value-of select="sag:Saglasnost/sag:Evidencija_o_vakcinaciji/sag:Revakcinacija/sag:Naziv_vakcine"/>
                                        </fo:block>
                                    </fo:table-cell>
                                    <fo:table-cell padding="2px">
                                        <fo:block>
                                            <xsl:value-of select="sag:Saglasnost/sag:Evidencija_o_vakcinaciji/sag:Revakcinacija/sag:Datum_vakcinacije"/>
                                        </fo:block>
                                    </fo:table-cell>
                                    <fo:table-cell padding="2px">
                                        <fo:block>
                                            <xsl:value-of select="sag:Saglasnost/sag:Evidencija_o_vakcinaciji/sag:Revakcinacija/sag:Naziv_vakcine"/>
                                        </fo:block>
                                    </fo:table-cell>
                                    <fo:table-cell padding="2px">
                                        <fo:block>
                                            <xsl:value-of select="sag:Saglasnost/sag:Evidencija_o_vakcinaciji/sag:Revakcinacija/sag:Ekstremitet"/>
                                        </fo:block>
                                    </fo:table-cell>
                                    <fo:table-cell padding="2px">
                                        <fo:block>
                                            <xsl:value-of select="sag:Saglasnost/sag:Evidencija_o_vakcinaciji/sag:Revakcinacija/sag:Lot"/>
                                        </fo:block>
                                    </fo:table-cell>
                                    <fo:table-cell padding="2px">
                                        <fo:block>
                                            <xsl:value-of select="sag:Saglasnost/sag:Evidencija_o_vakcinaciji/sag:Revakcinacija/sag:Proizvodjac"/>
                                        </fo:block>
                                    </fo:table-cell>
                                    <fo:table-cell padding="2px">
                                        <fo:block>
                                            <xsl:value-of select="sag:Saglasnost/sag:Evidencija_o_vakcinaciji/sag:Revakcinacija/sag:Nezeljena_reakcija"/>
                                        </fo:block>
                                    </fo:table-cell>
                                </fo:table-row>
                            </fo:table-body>
                        </fo:table>
                        <fo:table>
                            <fo:table-body>
                                <fo:table-row border="1px solid black">
                                    <fo:table-cell padding="2px">
                                        <fo:block>
                                            <fo:inline>
                                                Privremene kontraindikacije
                                            </fo:inline>
                                            <fo:inline>
                                                <xsl:value-of select="concat(' ',sag:Saglasnost/sag:Evidencija_o_vakcinaciji/sag:Privremene_kontraindikacije)"/>
                                            </fo:inline>
                                        </fo:block>
                                    </fo:table-cell>
                                </fo:table-row>
                                <fo:table-row border="1px solid black">
                                    <fo:table-cell padding="2px">
                                        <fo:block>
                                            <fo:inline>
                                                Odluka komisije za trajne kontraindikacije(Ako postoji upisati Da)
                                            </fo:inline>
                                            <fo:inline>
                                                <xsl:value-of select="concat(' ',sag:Saglasnost/sag:Evidencija_o_vakcinaciji/sag:Odluka_komisije)"/>
                                            </fo:inline>
                                        </fo:block>
                                    </fo:table-cell>
                                </fo:table-row>
                            </fo:table-body>
                        </fo:table>
                    </fo:block>
                </fo:flow>
            </fo:page-sequence>
        </fo:root>
    </xsl:template>
</xsl:stylesheet>